...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-navy shadow-nav">
      <nav className="home-container flex h-[60px] min-w-0 items-stretch justify-between gap-2 sm:h-[64px] lg:h-[68px]">
        <HomeSectionLink
          sectionId="home"
          className="nav-logo-slab relative z-10 -ml-4 flex shrink-0 items-center bg-white pl-4 pr-7 sm:-ml-6 sm:pl-6 sm:pr-9"
          onNavigate={closeMenu}
        >
          <Logo asImage className="[&_img]:h-7 sm:[&_img]:h-8" />
        </HomeSectionLink>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          <button
            type="button"
            className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg text-white lg:hidden"
            aria-expanded={open}
      ...
      {open && (
        <div className="border-t border-white/10 bg-navy-800 lg:hidden">
          <div className="home-container flex max-h-[70vh] flex-col gap-0.5 overflow-y-auto py-3">
            {navItems.map((item) => (
              <HomeSectionLink
                key={item.sectionId}
                sectionId={item.sectionId}
            ...
            <a
              href={`tel:${CONTACT.phoneTel}`}
              className="mt-1 inline-flex min-h-[44px] items-center gap-2 rounded-lg px-2 text-base font-medium text-white"
            >
