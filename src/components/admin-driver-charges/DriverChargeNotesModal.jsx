...
import { updateDriverCharge } from '../../lib/data/driverChargesRepository'
 ...
 * }} props
 */
export default function DriverChargeNotesModal({ open, charge, onClose, onSaved }) {
  ...
  async function save(e) {
    ...
    setErr('')
    try {
      await updateDriverCharge(String(charge.id), { notes: notes.trim() })
      await onSaved?.()
      onClose()
    } catch (ex) {
      setErr(ex?.message || 'Could not save notes.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-base font-bold text-slate-900">Edit charge notes</h3>
