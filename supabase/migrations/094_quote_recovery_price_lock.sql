-- Recovery resume: return locked price fields; never NULL-out estimated_total on upsert.
...
-- Preserve estimated_total when payload omits it or sends null/empty.
...
BEGIN
  ...
  IF v_row.id IS NULL THEN
    ...
    INSERT INTO public.customer_leads (
      ...
      CASE
        WHEN p_payload ? 'estimated_total'
          AND NULLIF(trim(p_payload->>'estimated_total'), '') IS NOT NULL
        THEN (p_payload->>'estimated_total')::numeric
        ELSE NULL
      END,
